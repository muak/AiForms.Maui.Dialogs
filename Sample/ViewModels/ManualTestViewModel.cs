...
public class ManualTestViewModel
{
    ...
    public ManualTestViewModel()
    {
        ...
        SaveCommand.Subscribe(_ =>
        {
            ...
            {
                Preferences.Set($"check{i}",Checks[i].Value);
            }           
        });

        for (var i = 0; i < Checks.Count; i++)
        {
            var check = Preferences.Get($"check{i}", false);                      
            Checks[i].Value = check;            
        }
    }
    ...
    async Task LoadingTest()
    {
        ...
        // IndicatorColor
        Configurations.LoadingConfig.IndicatorColor = Colors.Red;
        ...
        // FontColor
        Configurations.LoadingConfig.FontColor = Colors.Blue;
